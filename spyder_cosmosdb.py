    ...
    def process_each_commit(self,selected_commits):  # 循环，对每一个筛选完的，确认更新的链接点击进去并执行上面的函数对内容进行爬取
        ...
        for key in selected_commits:
            ...
            if commit_patch_data == "Error":
                ...
                gpt_summary_response = self.gpt_summary(input_dic, self.language, self.system_prompt_dict["GPT_SUMMARY_PROMPT"])
                if gpt_summary_response == None:
                    ...
                    gpt_title_response = self.gpt_title(gpt_summary_response, self.language, self.system_prompt_dict["GPT_TITLE_PROMPT"])
